...
public class UserTypeServiceImpl implements UserTypeService {
	...
	public UserTypeDto getUserType(Long id) {
		UserType userType = repository.findById(id).orElseThrow(() -> new UserTypeNotFoundException("User type not found"));
	...
	public UserTypeDto editUserType(Long id, UserTypeDto userTypeDto) {
		...
		if (!userType.getName().equals(userTypeDto.name()) && repository.findByName(userTypeDto.name()).isPresent()) {
			throw new UserTypeAlreadyExistsException("A user type with the name " + userTypeDto.name() + " already exists.");
		}
